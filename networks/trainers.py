...
class VQTokenizerTrainerV3(Trainer):
    ...
    def train(self, train_dataloader, val_dataloader, plot_eval, distri_eval=None):
        self.vq_encoder.to(self.device)
        self.quantizer.to(self.device)
        self.vq_decoder.to(self.device)
        self.discriminator.to(self.device)
        ...
        while epoch < self.opt.max_epoch:
            ...
            print('Validation Loss: %.5f' % (val_loss))
            self.logger.scalar_summary('epoch/val_loss', val_loss, epoch)
            ...
            if epoch % self.opt.eval_every_e == 0:
                # data = torch.cat([self.recon_motions[:4], self.motions[:4]], dim=0).detach().cpu().numpy()
                data = self.recon_motions[:4].detach().cpu().numpy()
                save_dir = pjoin(self.opt.eval_dir, 'E%04d_Rec' % (epoch))
                os.makedirs(save_dir, exist_ok=True)
                plot_eval(data, save_dir)
                data = self.motions[:4].detach().cpu().numpy()
                save_dir = pjoin(self.opt.eval_dir, 'E%04d_GT' % (epoch))
                os.makedirs(save_dir, exist_ok=True)
                plot_eval(data, save_dir)
            ...
            if epoch % self.opt.eval_every_e == 0:
                ...
                """
                if distri_eval is not None:
                    dist_result = distri_eval.eval_distrib(self.vq_encoder, self.quantizer)
                    for key in dist_result:
                        self.logger.scalar_summary('epoch/dist_{}'.format(key), dist_result[key], epoch)
...
class VQTokenizerTrainerV3_audio(Trainer):
    def __init__(self, args, vq_encoder, quantizer, vq_decoder, discriminator=None):
        """
        remove the :4 in motion input.
        """
        self.opt = args
        self.vq_encoder = vq_encoder
        self.vq_decoder = vq_decoder
